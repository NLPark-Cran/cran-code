    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[80dvh] max-w-[min(100vw-1.5rem,72rem)] flex-col gap-0 p-0 sm:h-[70vh] sm:max-w-6xl">
        <DialogHeader className="border-b px-4 py-3">
          <DialogTitle className="sr-only">{t("chat:searchMessagesTitle")}</DialogTitle>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {query
                ? matches.length > 0
                  ? t("chat:resultsCount", { count: matches.length })
                  : t("chat:noResults")
                : ""}
            </span>
          </div>
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            {selectedMatch ? (
              ...
              <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
                {query ? t("chat:selectResultPreview") : t("chat:typeToSearch")}
              </div>
            )}
          </div>
        </div>
...
function PreviewContent({
  ...
  if (message.content) {
    sections.push({ label: t("chat:sectionContent"), content: message.content });
  }
  if (message.thinking) {
    sections.push({ label: t("chat:sectionThinking"), content: message.thinking });
  }
  if (message.toolCall) {
    if (message.toolCall.title) {
      sections.push({ label: t("chat:sectionTool"), content: message.toolCall.title });
    }
    if (message.toolCall.input) {
      ...
      sections.push({ label: t("chat:sectionInput"), content: inputStr });
    }
    if (message.toolCall.output) {
      sections.push({ label: t("chat:sectionOutput"), content: message.toolCall.output });
    }
    if (message.toolCall.message) {
      sections.push({ label: t("chat:sectionMessage"), content: message.toolCall.message });
    }
  }
  if (message.codeSnippet?.code) {
    sections.push({ label: t("chat:sectionCode"), content: message.codeSnippet.code });
  }
