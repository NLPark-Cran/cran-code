    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label={t("chat:copyLabel", { label })}
        >
          {copied ? (
...
export function SessionInfoSection({
  ...
  return (
    <div className="space-y-3">
      <p className="font-medium text-sm">{t("chat:sessionInfo")}</p>
      <SessionInfoItem label={t("chat:sessionId")} value={sessionId} />
      {session?.workDir && (
        <SessionInfoItem label={t("chat:workingDir")} value={session.workDir} />
      )}
      {session?.sessionDir && (
        <SessionInfoItem label={t("chat:sessionDir")} value={session.sessionDir} />
      )}
    </div>
  );
}
  ...
  return (
    <DropdownMenu>
      <Tooltip>
        ...
        <TooltipContent side="bottom">{t("chat:sessionInfo")}</TooltipContent>
      </Tooltip>
