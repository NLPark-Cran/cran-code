...
def _version_callback(value: bool) -> None:
    if value:
        from cran_code.constant import get_version
...
def kimi(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.cran/config.toml.",
        ),
    ] = None,
    ...
    from cran_code.utils.proctitle import init_process_name
    ...
    from cran_code.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from cran_code.app import KimiCLI, enable_logging
    from cran_code.config import Config, load_config_from_string
    from cran_code.exception import ConfigError
    from cran_code.hooks import events as hook_events
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from cran_code.utils.path import is_within_directory
    ...
    if _picker_mode:
        ...
        from cran_code.utils.datetime import format_relative_time
    ...
    if switch_target in ("web", "vis"):
        from cran_code.utils.logging import restore_stderr
        ...
        from cran_code.utils.term import ensure_tty_sane

        ensure_tty_sane()

        if switch_target == "web":
            from cran_code.web.app import run_web_server

            run_web_server(open_browser=True)
        else:
            from cran_code.vis.app import run_vis_server
...
def login(
    ...
    from cran_code.auth.oauth import login_kimi_code
...
def logout(
    ...
    from cran_code.auth.oauth import logout_kimi_code
    from cran_code.config import load_config
...
def acp():
    ...
    from cran_code.acp import acp_main
...
if __name__ == "__main__":
    ...
    if "cran_code.cli" not in sys.modules:
        sys.modules["cran_code.cli"] = sys.modules[__name__]
