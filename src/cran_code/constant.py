...
def get_build_sha() -> str:
    ...
    Priority:
    1. CRAN_BUILD_SHA environment variable (dev / CI override)
    ...
    """
    if build_id := os.environ.get("CRAN_BUILD_SHA", "").strip():
        return build_id

    try:
        from cran_code._build_info import BUILD_SHA  # type: ignore[reportMissingImports]
