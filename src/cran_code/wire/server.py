...
class WireServer:
    ...
    async def _handle_initialize(
        ...
        from cran_code.constant import NAME, VERSION
        from cran_code.hooks.config import HOOK_EVENT_TYPES
        from cran_code.hooks.engine import WireHookHandle, WireHookSubscription
        from cran_code.soul import wire_send
        from cran_code.wire.protocol import WIRE_PROTOCOL_VERSION
        from cran_code.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from cran_code.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from cran_code.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from cran_code.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        ...
        if isinstance(llm.chat_provider, Kimi):
            cran_codeent = llm.chat_provider.client
            headers = dict(cran_codeent._custom_headers)  # pyright: ignore[reportPrivateUsage]
            headers["User-Agent"] = f"{USER_AGENT}{ua_suffix}"
            cran_codeent._custom_headers = headers  # pyright: ignore[reportPrivateUsage]
