...
def augment_provider_with_env_vars(provider: LLMProvider, model: LLMModel) -> dict[str, str]:
    ...
    match provider.type:
        case "kimi":
            if base_url := os.getenv("CRAN_BASE_URL"):
                provider.base_url = base_url
                applied["CRAN_BASE_URL"] = base_url
            if api_key := os.getenv("CRAN_API_KEY"):
                provider.api_key = SecretStr(api_key)
                applied["CRAN_API_KEY"] = "******"
            if model_name := os.getenv("CRAN_MODEL_NAME"):
                model.model = model_name
                applied["CRAN_MODEL_NAME"] = model_name
