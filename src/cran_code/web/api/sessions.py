...
from cran_code.web.runner.messages import new_session_status_message, send_history_complete
from cran_code.web.runner.process import KimiCLIRunner
from cran_code.web.store.sessions import (
    JointSession,
...
from cran_code.wire.types import is_request
...
async def create_session(request: CreateSessionRequest | None = None) -> Session:
    ...
    return Session(
        session_id=UUID(cran_code_session.id),
        title=cran_code_session.title,
...
async def get_session_file(
    ...
    work_dir = Path(str(session.cran_code_session.work_dir)).resolve()
...
def _update_last_session_id(session: JointSession) -> None:
    ...
    kimi_session = session.cran_code_session
...
async def delete_session(session_id: UUID, runner: KimiCLIRunner = Depends(get_runner)) -> None:
    ...
    wd_meta = session.cran_code_session.work_dir_meta
