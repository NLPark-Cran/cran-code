...
Usage:
    python -m cran_code.web.runner.worker <session_id>
"""
...
async def run_worker(session_id: UUID) -> None:
    ...
    try:
        cran_code = await KimiCLI.create(
    ...
    await cran_code.run_wire_stdio()
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m cran_code.web.runner.worker <session_id>", file=sys.stderr)
        sys.exit(1)
