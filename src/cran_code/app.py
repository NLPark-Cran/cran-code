...
from cran_code.auth.oauth import KIMI_CODE_OAUTH_KEY, OAuthManager, get_device_id
from cran_code.background.models import is_terminal_status
from cran_code.cli import InputFormat, OutputFormat
from cran_code.config import Config, LLMModel, LLMProvider, load_config
from cran_code.constant import VERSION
from cran_code.llm import augment_provider_with_env_vars, create_llm, model_display_name
from cran_code.session import Session
from cran_code.share import get_share_dir
from cran_code.soul import RunCancelled, run_soul
...
from cran_code.utils.envvar import get_env_bool
from cran_code.utils.logging import logger, open_original_stderr, redirect_stderr_to_logger
from cran_code.utils.path import shorten_home
from cran_code.wire import Wire, WireUISide
from cran_code.wire.types import ApprovalRequest, ApprovalResponse, ContentPart, WireMessage
...
def _patch_session_id(record: dict[str, Any]) -> None:
    ...
    try:
        from cran_code.soul.toolset import get_session_id
...
async def _refresh_managed_models_silent(config: Config) -> None:
    from cran_code.auth.platforms import refresh_managed_models
...
class KimiCLI:
    ...
    async def create(
        ...
        *,
        ...
        from cran_code.telemetry import attach_sink, set_context
        from cran_code.telemetry import disable as disable_telemetry

        telemetry_disabled = not config.telemetry or get_env_bool("CRAN_DISABLE_TELEMETRY")
        if telemetry_disabled:
            disable_telemetry()
        else:
            device_id = get_device_id()
            set_context(device_id=device_id, session_id=session.id)
            from cran_code.telemetry.sink import EventSink
            from cran_code.telemetry.transport import AsyncTransport
        ...
        from cran_code.telemetry import track, track_session_started_once
        from cran_code.telemetry.crash import install_asyncio_handler, set_phase
    ...
    async def run_shell(
        ...
        from cran_code.ui.shell import Shell, WelcomeInfoItem

        if command is None:
            from cran_code.ui.shell.update import check_update_gate
        ...
        if base_url := self._env_overrides.get("CRAN_BASE_URL"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API URL",
                    value=f"{base_url} (from CRAN_BASE_URL)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        if self._env_overrides.get("CRAN_API_KEY"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API Key",
                    value="****** (from CRAN_API_KEY)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        ...
        elif "CRAN_MODEL_NAME" in self._env_overrides:
            welcome_info.append(
                WelcomeInfoItem(
                    name="Model",
                    value=f"{self._soul.model_name} (from CRAN_MODEL_NAME)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        else:
    ...
    async def run_print(
        ...
        *,
        ...
        from cran_code.ui.print import Print
    ...
    async def run_acp(self) -> None:
        ...
        from cran_code.ui.acp import ACP
