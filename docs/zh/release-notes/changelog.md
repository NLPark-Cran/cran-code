...
- Soul：在上游「按剩余上下文计算补全预算」之上，保留分支的压缩保留尾部 token 预算（修复上下文用量超过 100% 的问题）；当整段历史摘要后仍无法降到安全阈值以下时，以明确错误拒绝替换上下文
- Web UI：将 `context_usage` 钳制在 [0, 100%] 区间内，后端异常不会再渲染出不可能的百分比
