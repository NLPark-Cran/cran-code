...
def _make_oauth_manager(config: Config, initial_token: OAuthToken | None = None) -> OAuthManager:
    ...
    with patch("cran_code.auth.oauth.load_tokens", return_value=initial_token):
...
def test_resolve_api_key_falls_back_to_api_key_when_no_token():
    ...
    with patch("cran_code.auth.oauth.load_tokens", return_value=None):
...
def test_resolve_api_key_falls_back_when_token_has_empty_access_token():
    ...
    with patch("cran_code.auth.oauth.load_tokens", return_value=empty_token):
...
async def test_ensure_fresh_without_runtime_caches_token():
    ...
    with patch("cran_code.auth.oauth.load_tokens", return_value=fresh_token):
...
async def test_ensure_fresh_without_runtime_refreshes_expired_token():
    ...
    with (
        patch("cran_code.auth.oauth.load_tokens", return_value=expired_token),
        patch(
            "cran_code.auth.oauth.refresh_token",
