...
from cran_code.approval_runtime import get_current_approval_source_or_none
from cran_code.soul import MaxStepsReached, RunCancelled
...
from cran_code.wire.types import ApprovalRequest, TextPart
...
async def test_foreground_agent_explicit_timeout_returns_tool_error(
    ...
    monkeypatch.setattr("cran_code.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("cran_code.subagents.runner.run_soul", fake_run_soul_hang)
...
async def test_foreground_agent_internal_timeout_with_explicit_deadline(
    ...
    monkeypatch.setattr("cran_code.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("cran_code.subagents.runner.run_soul", fake_run_soul_internal_timeout)
...
async def test_agent_tool_returns_error_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "cran_code.subagents.runner.run_with_summary_continuation",
...
async def test_background_agent_marks_failed_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "cran_code.background.agent_runner.run_with_summary_continuation",
        fake_run_with_summary,
    )
...
async def test_foreground_runner_hook_trigger_exception_marks_instance_failed(
    ...
    monkeypatch.setattr(
        "cran_code.hooks.events.subagent_start",
