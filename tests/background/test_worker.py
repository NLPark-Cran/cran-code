...
from cran_code.background import (
    BackgroundTaskStore,
    TaskControl,
    TaskSpec,
    run_background_task_worker,
)
from cran_code.background.worker import terminate_process_tree_windows
...
def test_terminate_process_tree_windows_uses_taskkill_tree(monkeypatch):
    ...
    monkeypatch.setattr("cran_code.background.worker.subprocess.run", _run)
