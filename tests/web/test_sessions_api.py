...
from cran_code.web.api import sessions as sessions_api
from cran_code.web.api.sessions import SESSION_TITLE_MAX_COMPLETION_TOKENS
from cran_code.web.models import GenerateTitleRequest
...
async def test_generate_title_preserves_concurrent_manual_title(
    ...
    monkeypatch.setattr("cran_code.config.load_config", lambda: config)
    monkeypatch.setattr(
        "cran_code.llm.create_llm",
        lambda provider_config, model_config, oauth=None: _FakeLLM(),
    )
    monkeypatch.setattr("cran_code.auth.oauth.OAuthManager", _FakeOAuthManager)
    ...
    response = await sessions_api.generate_session_title(
        ...
        current_user=cast("CurrentUser", SimpleNamespace(id="test-user")),
    )
...
async def test_generate_title_caps_kimi_completion(
    ...
    monkeypatch.setattr("cran_code.llm.with_kimi_generation_overrides", fake_with_overrides)
    monkeypatch.setattr("kosong.generate", fake_generate)
