...
def test_import_cran_code_does_not_import_loguru() -> None:
    proc = _run_python(
        """
import sys
...
def test_import_cran_code_constant_defers_package_metadata() -> None:
    proc = _run_python(
        """
import sys
sys.modules.pop("importlib.metadata", None)
import cran_code.constant as constant
...
assert result.exit_code == 0, result.output
assert '"cran_code_version"' in result.output
assert "cran_code.cli.info" in sys.modules
assert "cran_code.cli.export" not in sys.modules
assert "cran_code.cli.mcp" not in sys.modules
assert "cran_code.cli.vis" not in sys.modules
assert "cran_code.cli.web" not in sys.modules
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
sys.modules.pop("cran_code.cli", None)

from cran_code.__main__ import main
...
assert "cran_code.cli" not in sys.modules
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
assert "cran_code.cli" in sys.modules
payload = json.loads(stdout.getvalue())
assert payload["cran_code_version"]
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
import cran_code.cli.__main__ as cli_main
