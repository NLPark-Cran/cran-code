...
from cran_code.approval_runtime import ApprovalSource, get_current_approval_source_or_none
from cran_code.background import TaskRuntime, TaskSpec
from cran_code.llm import LLM
from cran_code.notifications import NotificationEvent
from cran_code.soul import RunCancelled, StatusSnapshot, _current_wire, run_soul
...
from cran_code.wire.types import ApprovalRequest, ApprovalResponse, Notification
...
@pytest.mark.asyncio
async def test_cran_code_run_yields_root_hub_approvals(runtime: Runtime) -> None:
...
@pytest.mark.asyncio
async def test_cran_code_run_bridges_approval_resolution_back_to_runtime(runtime: Runtime) -> None:
...
@pytest.mark.asyncio
async def test_cran_code_run_cancels_abandoned_approval_stream(
    runtime: Runtime,
    tmp_path: Path,
...
@pytest.mark.asyncio
async def test_cran_code_run_propagates_external_cancel_event(
    runtime: Runtime,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_cran_code_run_replays_pending_approvals_from_previous_turn(runtime: Runtime) -> None:
    assert runtime.approval_runtime is not None
