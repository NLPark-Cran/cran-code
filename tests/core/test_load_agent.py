...
from cran_code.exception import InvalidToolError, SystemPromptTemplateError
from cran_code.session import Session
from cran_code.soul.agent import BuiltinSystemPromptArgs, Runtime, _load_system_prompt, load_agent
...
def test_load_tools_valid(runtime: Runtime):
    ...
    tool_paths = ["cran_code.tools.think:Think", "cran_code.tools.shell:Shell"]
    toolset = KimiToolset()
    toolset.load_tools(
        tool_paths,
        {
...
def test_load_tools_invalid(runtime: Runtime):
    ...
    tool_paths = ["cran_code.tools.nonexistent:Tool", "cran_code.tools.think:Think"]
    toolset = KimiToolset()
    try:
        toolset.load_tools(
            tool_paths,
        ...
        assert "cran_code.tools.nonexistent:Tool" in str(e)
...
agent:
  name: "Test Agent"
  system_prompt_path: ./system.md
  tools: ["cran_code.tools.nonexistent:Tool"]
""")
