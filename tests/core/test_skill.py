...
async def test_resolve_skills_roots_uses_layers(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRAN_SHARE_DIR", str(tmp_path / "share"))
...
async def test_find_user_skills_dirs_empty_generic_does_not_shadow_brand(monkeypatch, tmp_path):
    """Core bug: empty ~/.config/agents/skills should NOT hide ~/.cran/skills."""
    home_dir = tmp_path / "home"
    generic_dir = home_dir / ".config" / "agents" / "skills"
    generic_dir.mkdir(parents=True)  # exists but empty

    brand_dir = home_dir / ".cran" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_brand_group_prefers_cran_over_claude(monkeypatch, tmp_path):
    """Brand group: ~/.cran/skills takes priority over ~/.claude/skills."""
    home_dir = tmp_path / "home"
    cran_dir = home_dir / ".cran" / "skills"
    cran_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home_dir)

    dirs = await find_user_skills_dirs()
    # Only cran should be selected (first existing in brand group)
    assert KaosPath.unsafe_from_local_path(cran_dir) in dirs
...
async def test_find_project_skills_dirs_merge(tmp_path):
    ...
    generic_dir.mkdir(parents=True)
    brand_dir = work_dir / ".cran" / "skills"
    brand_dir.mkdir(parents=True)
...
@pytest.mark.asyncio
async def test_find_project_skills_dirs_brand_prefers_cran(tmp_path):
    """Project layer brand group: .cran/skills wins over .claude/skills."""
    work_dir = tmp_path / "project"
    cran_dir = work_dir / ".cran" / "skills"
    cran_dir.mkdir(parents=True)
    ...
    dirs = await find_project_skills_dirs(KaosPath.unsafe_from_local_path(work_dir))
    assert len(dirs) == 1
    assert dirs[0] == KaosPath.unsafe_from_local_path(cran_dir)
...
async def test_resolve_skills_roots_merges_user_and_project(monkeypatch, tmp_path):
    ...
    user_brand = home_dir / ".cran" / "skills"
    ...
    proj_brand = work_dir / ".cran" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_cran_and_claude(monkeypatch, tmp_path):
    """merge_brands=True: cran + claude both exist → both returned, cran first."""
    home_dir = tmp_path / "home"
    cran_dir = home_dir / ".cran" / "skills"
    cran_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
...
async def test_find_user_skills_dirs_merge_brands_all_three(monkeypatch, tmp_path):
    ...
    assert brand_dirs[0] == KaosPath.unsafe_from_local_path(cran_dir)
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_same_skill_cran_wins(monkeypatch, tmp_path):
    """merge_brands=True + same skill name → cran version wins via discover."""
    home_dir = tmp_path / "home"
    cran_dir = home_dir / ".cran" / "skills"
    cran_dir.mkdir(parents=True)
    _write_skill(
        cran_dir / "deploy",
        "---\nname: deploy\ndescription: cran deploy\n---\n",
    )
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    _write_skill(
    ...
    skills = await discover_skills_from_roots(_as_user(dirs))
    assert len(skills) == 1
    assert skills[0].description == "cran deploy"
...
def test_get_builtin_skills_dir_frozen_env(monkeypatch, tmp_path):
    ...
    assert result == fake_meipass / "cran_code" / "skills"
...
def test_get_builtin_skills_dir_normal_env():
    ...
    assert result.parent.name == "cran_code"
...
async def test_resolve_skills_roots_passes_merge_brands(monkeypatch, tmp_path):
    ...
    # Without merge_brands: only cran
    roots_default = _roots_of(await resolve_skills_roots(KaosPath.unsafe_from_local_path(work_dir)))
    assert KaosPath.unsafe_from_local_path(cran_dir) in roots_default
    ...
    assert KaosPath.unsafe_from_local_path(cran_dir) in roots_merged
...
async def test_project_scope_skill_overrides_builtin_with_same_name(monkeypatch, tmp_path):
    ...
    import cran_code.skill as skill_mod
...
async def test_extra_skill_dirs_dedup_with_auto_discovery_overlap(monkeypatch, tmp_path):
    ...
    Scenario: user puts ``~/.cran/skills`` (already auto-discovered as the
...
# the work directory. Launching cran-code from a subdirectory of a monorepo
...
async def test_find_project_skills_dirs_walks_up_to_git_root(tmp_path):
    ...
    defined at the repo root — otherwise running cran-code inside a monorepo
...
# ~/.cran/skills and ~/.claude/skills after migrating from Claude Code)
# previously lost visibility of everything in .claude/skills because the
# default "first brand wins" behaviour would stop at .cran/skills. Flipping
...
async def test_default_config_effectively_merges_user_brand_skill_dirs(monkeypatch, tmp_path):
    ...
    both ``~/.cran/skills`` and ``~/.claude/skills`` are surfaced. If the
    ...
    assert KaosPath.unsafe_from_local_path(cran_dir) in roots
