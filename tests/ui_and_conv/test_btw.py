...
from cran_code.soul.btw import (
    _build_btw_context,
    _DenyAllToolset,
...
class TestExecuteSideQuestion:
    ...
    def test_text_on_first_turn(self):
        ...
        with patch("cran_code.soul.btw.kosong.step", side_effect=fake_step):
            response, error = asyncio.run(execute_side_question(soul, "hi"))
    ...
    def test_applies_kimi_request_provider_from_soul(self):
        ...
        with (
            patch("cran_code.soul.btw.kosong.step", side_effect=fake_step),
            patch(
                "cran_code.soul.btw.with_kimi_generation_overrides",
...
class TestHandleLocalInput:
    ...
    def test_shell_command_blocked_from_queue(self, monkeypatch):
        ...
        toasted = []
        monkeypatch.setattr(
            "cran_code.ui.shell.prompt.toast",
