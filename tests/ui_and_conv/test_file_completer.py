...
from cran_code.ui.shell.prompt import LocalFileMentionCompleter
...
def test_completed_file_short_circuits_completions(tmp_path: Path):
    ...
    nested_dir = tmp_path / "src" / "cran_code" / "agents"
...
def test_basename_prefix_is_ranked_first(tmp_path: Path):
    ...
    (tmp_path / "src" / "cran_code" / "tools" / "web").mkdir(parents=True)
    (tmp_path / "src" / "cran_code" / "tools" / "file").mkdir(parents=True)

    fetch_py = tmp_path / "src" / "cran_code" / "tools" / "web" / "fetch.py"
    fetch_py.write_text("# fetch\n")
    patch_py = tmp_path / "src" / "cran_code" / "tools" / "file" / "patch.py"
    ...
    assert texts == snapshot(
        [
            "src/cran_code/tools/web/fetch.py",
            "src/cran_code/tools/file/patch.py",
        ]
    )
